...
Scenario: Get all articles
    ...
    When method Get
    Then status 200
    And match response == {"articles": '#array', "articlesCount": 199}
    And match response.articles == '#[10]' //number of array inside articles
    ...
    And match each response..bio == '#present' //validate bio key must be present with any type of value including null
    And match each response..bio == '##number' //if key present then validates for value else ignores the key
